...
class XOR:
    ...
    def encrypt(self,text:str) -> str:
        result=''
        counter=0
        vi=text_to_bits(str(random.randint(100,999)))
        for b in text_to_bits(text):
            result += "0"if b == (vi + self.key)[counter] else "1"
            counter += 1
            if counter >=len(self.key + vi):
                counter = 0 
        return text_from_bits(result) + "".join(chr(ord(i) +70)for i in text_from_bits(vi))
    def decrypt(self, text:str) -> str:
        result = ""
        counter = 0
        vi = text_to_bits("".join((chr(ord(i) - 70)
            for i in text[len(text) - 3 :])))
        for b in text_to_bits(text[:-3]):
            result += "0"if b == (vi + self.key)[counter] else "1"
            counter += 1
            if counter >= len(self.key + vi):
                counter = 0
...
def text_to_bits(text):
    result = ""
    for i in text:
        result += format(ord(i), "b").zfill(8)
    return result

def text_from_bits(bits):
    result = ""
    f = 0
    for i in range(8, len(bits) + 8,8):
        result += chr(int(bits[f:i], 2))
        f = i
    return result
